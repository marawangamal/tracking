...
def eval_seq(opt, dataloader, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30):
    ...
    # # seek to last processed frame
    # print("seeeking...")
    # for i in tqdm(range(len(dataloader))):
    #     if not os.path.exists(os.path.join(save_dir, '{:05d}.jpg'.format(frame_id))):
    #         break
    #     else:
    #         frame_id +=1
    #         i+=1
    # print("skipped {} frames".format(i))

    for sample in tqdm(dataloader):
        i, (path, img, img0) = i+1, sample
        #if i % 8 != 0:
            #continue
        # if frame_id % 20 == 0:
        ...
        if os.path.exists(os.path.join(save_dir, '{:05d}.jpg'.format(frame_id))):
            continue
